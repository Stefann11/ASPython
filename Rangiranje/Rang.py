...
def odrediRang(dictStranica, rangStranice, imenaStranicaSaLinkovima):
    ...
    for key1 in imenaStranicaSaLinkovima.keys():         #idi po svim kljucevima tj. linkovima
        for value1 in imenaStranicaSaLinkovima[key1]:    #idi po svim vrednostima tj. html stranicama koje sadrze te linkove
    ...
    #print("ZA LINKOVE")
    for key, value in rangStranice.items():     #rangStranice sadrzi html stranice za kljuceve i broj njihovih pojavljivanja kao linkova za vrednost
        ...
        for key2, value2 in dictStranica.items():       #dictStranica za kljuceve ima html stranice a za vrednosti broj trazenih reci na njima
            if key2.replace("/", "\\") == key.replace("/", "\\"):       #algoritam za proveravanje da li se neki link pojavljuje i kao cvor
                if key2 not in rezultujuciDict.keys():
                    rezultujuciDict[key2] = 2*value     #ako se ovaj kljuc ne nalazi u rezultujuciDict dodaj ga (ovaj slucaj vrednujemo vise u ukupnom rangu, pa mu vrednost mnozimo sa 2)
                else:
                    rezultujuciDict[key2] += 2*value    #ako se vec nalazi u rezulujuciDict, samo mu povecaj vrednost



    #print("ZA RECI U LINKOVIMA")
    for key, value in dictRang.items():     #dictRang kao kljuc sadrzi html stranice, a kao vrednost broj pojavljivanja trazene reci na njima
        #print(key,value)#za reci u linkovima
        if key not in rezultujuciDict.keys():
            rezultujuciDict[key] = value    #ako se ovaj kljuc ne nalazi u rezultujuciDict
        else:
            rezultujuciDict[key] += value   #ako se ovaj kljuc nalazi u rezultujuciDict, uvecaj vrednost
